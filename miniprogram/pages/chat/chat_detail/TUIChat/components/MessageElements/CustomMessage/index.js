  ...
  methods: {
    ...
    parseCustom(message) {
      ...
      try {
        const callingmessage = JSON.parse(message.payload.data);
        if (callingmessage.businessID === 1) {
          if (message.conversationType === wx.chat.TYPES.CONV_GROUP) {
          ...
          if (message.conversationType === wx.chat.TYPES.CONV_C2C) {
    ...
    openOrder(e) {
      const link = e.currentTarget.dataset.value.link;
      if (link){
        wx.navigateTo({
          url: link,
        }).catch((e) => {
          console.error("商品页面跳转错误：" + e);
        });
      }
    }
  },
