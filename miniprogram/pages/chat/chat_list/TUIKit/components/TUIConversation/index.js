...
// TUIKitWChat/Conversation/index.ts
const app = getApp();
...
Component({
  ...
  lifetimes: {
    detached() {
      tim.off(tim.EVENT.CONVERSATION_LIST_UPDATED, this.onConversationListUpdated, this);
      tim.off(tim.EVENT.USER_STATUS_UPDATED, this.onUserStatusUpdate, this);
      this.setData({
        isInit: false,
      })
    },
  ...
  methods: {
    ...
    initEvent() {
      if (!this.data.isInit) {
        tim.on(tim.EVENT.CONVERSATION_LIST_UPDATED, this.onConversationListUpdated, this);
        tim.on(tim.EVENT.USER_STATUS_UPDATED, this.onUserStatusUpdate, this);
    ...
    filterUserIDList(conversationList) {
      ...
      conversationList.forEach((element) => {
        if (element.type === tim.TYPES.CONV_C2C) {
    ...
    subscribeOnlineStatus(userIDList) {
      ...
      tim.getUserStatus({ userIDList }).then((imResponse) => {
      ...
      tim.subscribeUserStatus({ userIDList });
    },
    learnMore() {
